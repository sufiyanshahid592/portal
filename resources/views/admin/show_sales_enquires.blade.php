...
<div class="form-element-area">
						...
						<!-- <div class="row"> -->
								...
								<div class="breadcomb-wp" style="float:right">
									...
									<div class="breadcomb-ctn">
									  <a href="{{url('own-sales-enquiry')}}" target="_blank"> <button class="custom-btn_hov btn-12 button_hov"><span>Click</span><span>My own Sales Enquiries</span></button>	</a>
                                    ...
                                    <!-- <div class="breadcomb-ctn"> -->
                                        <!-- <div class="row"> -->
                                            <div class="col-lg-6">
                                                <a href="{{url('your-tl-inquiries')}}" target="_blank"><button class="custom-btn_hov btn-12 button_hov"><span>Click</span><span>Get Your TL Inquiries</span></button></a>
                                            </div>
                                            <div class="col-lg-6">
                                                <a href="{{url('your-csr-inquiries')}}" target="_blank"><button class="custom-btn_hov btn-12 button_hov"><span>Click</span><span>Get Your CSR Inquiries</span></button></a>
                                            </div>
                                        <!-- </div> -->
                                        <br>
    ...
    <!-- <button type="button"  data-toggle="modal" data-target="#exampleModal"  data-product="{{$value->product}}"  data-quantity="{{$value->qty}}" data-price="{{$value->price}}" data-stock="{{$value->stock}}" data-size="{{$value->open_size_1}}" data-id="{{$value->id}}" data-finish="{{$value->f_s_1}}" data-width="{{$value->f_s_1}}"  data-printing="{{$value->printing}}" value="" data-id="{{$value->id}}" class="addAttr modaldatabtn btn-success notika-btn-success px-sm-4 get-product-info-view" > Product Info</button> -->
...
<!-- Modal -->
                      ...
                      <td class="text-center">
                        ...
                        <a href="{{url('edit-inquiry/'.$value->id)}}" title="delete" target="_blank" class="delete" 
   onclick="return confirm('Are you sure you want to Edit this Inquiry')">
  ...
  Ready Price Quote
    ...
    <a href="{{url('time-increase-sm'.'/'.$value->id)}}" class="@if($value->time_increment == 1)<?php echo"disable"?>@endif">
                      ...
                      <td class="text-center">
                        ...
                        <a href="{{url('edit-inquiry/'.$value->id)}}" target="_blank" title="delete" class="delete" 
   onclick="return confirm('Are you sure you want to Edit this Inquiry')">
          <button
                          class="btn"
                          data-toggle="tooltip"
...
if (window.location.host != "s.codepen.io") {
 ...
 <script>
  ...
  if(this.value=="est"){
     ...
     if (ok.value) {
  ...
  }else if(this.value=="upr"){
        var selectedid = $(this).find(':selected').attr('data-id');
        var csrf_token = $("meta[name=csrf-token]").attr("content");
        $.ajax({
            url: "{{url('urgent-price-request')}}",
            method: "post",
            data: {selectedid:selectedid, "_token":csrf_token},
            success: function(succ){
      ...
      $(document).on("click", ".follow-btn", function(){
        ...
        if(follow_data_id==1){
            $(".follow-textarea textarea").html("HI, Just for the confirmation (important)\nHow are you doing today!\nThis is to confirm you if you have got my previous email along with the Prices.\nPlease do let me know if we are good to proceed further.\nThank You!\nFeel free to ask any assistance.\nRegards,\
                ");
        }else if(follow_data_id==2){
            $(".follow-textarea textarea").html("Hi Scott,\nhope you are doing fine. This is just a follow up.  If you have any questions do let us know, we will be happy to assist you.\nWe have added your name in our client board list, so it’s our duty to remind you.\nMay I know your final decision about those boxes and your plan that when you are going to place the Order.\nI would like to hear soon from your side.\nFeel free to ask any questions.\nBest Regards,\nDavis Elton\nAccount Executive\nwww.thecustomboxes.com\nToll Free: 1800-396-1840\
");
        }else if(follow_data_id==3){
            $(".follow-textarea textarea").html("Hi,\nI hope you doing fine. This is just a follow up. From couple of days we didn’t hear a single word from your side. Is there any pricing issue, or don’t you have your designs and art work ready yet. If not, then let us know we can do that Free of cost.\nKindly, share your thoughts with us. We would like to hear soon from your side.\nFeel free to ask for any assistance.\nBest Regards,\nDavis Elton\nAccount Executive\nwww.thecustomboxes.com\nToll Free: 1800-396-1840\
");
        }
      });
